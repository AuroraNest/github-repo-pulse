          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium ${active ? "bg-blue-50 text-blue-700" : "text-slate-600 hover:bg-slate-100 hover:text-slate-950"}`}
              >
                <item.icon size={18} />
...
function isActiveNavItem(pathname: string, href: string) {
  ...
  return href === "/repositories" && pathname.startsWith("/repositories/");
}
