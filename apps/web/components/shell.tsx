...
export function AppShell({ children, labels, locale }: { children: ReactNode; labels: Dictionary; locale: Locale }) {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950">
        ...
        <div className="mt-8 space-y-3">
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            ...
            <div className="mt-3 flex items-center justify-between">
              <span className="text-xs text-slate-500">{labels.common.mockFallback}</span>
              <Chip tone="green">{labels.common.healthy}</Chip>
            </div>
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-700">
      ...
      <div className="lg:pl-60">
        <Topbar labels={labels} locale={locale} />
...
function Topbar({ labels, locale }: { labels: Dictionary; locale: Locale }) {
  return (
    <header className="sticky top-0 z-20 border-b border-slate-200 bg-slate-50/85 px-4 py-3 backdrop-blur-xl sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Chip tone="blue">{labels.common.trackingRepos}</Chip>
          <Chip tone="slate">{labels.common.lastSync}</Chip>
