...
- 图表：Recharts
- 表格：TanStack Table
- 数据库：当前实现选择 MySQL-first，抽象 DB 层，后续支持 PostgreSQL/SQLite adapter
- ORM/DB：可用 Drizzle/Prisma 或轻量 query helper，但业务层不得直接拼数据库 SQL
...
- Docker Compose：一键启动 web + worker + MySQL volume
